                ...
                size_t count, loff_t *offset)
{
        ...
        if (*offset > 0) {
                ...
                if (strcmp(name, strA) == 0) {
                        ...
                        dataStog = (struct abfunction *)filp->private_data;
...
static struct dentry *myfs_create_func_file(struct super_block *sb, struct dentry *dir, const char *name, struct abfunction *counter) {
    struct dentry *dentry;
    struct inode *inode;
    struct qstr qname;
...
static void myfs_create_files (struct super_block *sb, struct dentry *root)
{
        ...
        if (outputdir) {
                myfs_create_func_file(sb, outputdir, "add", &abfunction);
                myfs_create_func_file(sb, outputdir, "sub", &abfunction);
        }
}
